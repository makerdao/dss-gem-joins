...
import {WrappedGem} from "./wgem.sol";
...
contract DssDeployTest is DssDeployTestBase {
    ...
    function testGemJoin_PAXG() public {
        ...
        WrappedGem wpaxg = new WrappedGem(address(paxg));
        GemJoin paxgJoin = new GemJoin(address(vat), "PAXG", address(wpaxg));
        wpaxg.rely(address(paxgJoin));

        dssDeploy.deployCollateral("PAXG", address(paxgJoin), address(pip));

        paxg.approve(address(wpaxg), uint256(-1));
        assertEq(paxg.balanceOf(address(this)), 100 * 10 ** 18);
        assertEq(paxg.balanceOf(address(paxgJoin)), 0);
        assertEq(paxg.balanceOf(address(wpaxg)), 0);
        assertEq(wpaxg.balanceOf(address(this)), 0);
        assertEq(wpaxg.balanceOf(address(paxgJoin)), 0);
        assertEq(vat.gem("PAXG", address(this)), 0);
        wpaxg.mint(address(this), 100 * 10 ** 18);
        assertEq(paxg.balanceOf(address(this)), 0);
        assertEq(paxg.balanceOf(address(paxgJoin)), 0);
        assertEq(paxg.balanceOf(address(wpaxg)), 100 * 10 ** 18);
        assertEq(wpaxg.balanceOf(address(this)), 100 * 10 ** 18);
        assertEq(wpaxg.balanceOf(address(paxgJoin)), 0);
        paxgJoin.join(address(this), 10 * 10 ** 18);
        assertEq(paxg.balanceOf(address(wpaxg)), 100 * 10 ** 18);
        assertEq(wpaxg.balanceOf(address(this)), 90 * 10 ** 18);
        assertEq(wpaxg.balanceOf(address(paxgJoin)), 10 * 10 ** 18);
        assertEq(vat.gem("PAXG", address(this)), 10 ether);
        paxgJoin.exit(address(this), 4 * 10 ** 18);
        assertEq(paxg.balanceOf(address(wpaxg)), 100 * 10 ** 18);
        assertEq(wpaxg.balanceOf(address(this)), 94 * 10 ** 18);
        assertEq(wpaxg.balanceOf(address(paxgJoin)), 6 * 10 ** 18);
